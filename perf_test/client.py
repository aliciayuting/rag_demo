...
def main(argv):
     ...
     tl = TimestampLogger()
     res = capi.create_object_pool("/first",SUBGROUP_TYPES["VCSS"], 0)
     if res:
          res.get_result()
     else:
          print(f"Failed to create object pool")

     res = capi.create_object_pool("/second",SUBGROUP_TYPES["VCSS"], 0)
     if res:   
          res.get_result()
     else:
          print(f"Failed to create object pool")
     res = capi.create_object_pool("/final",SUBGROUP_TYPES["VCSS"], 0)
     if res:
          res.get_result()
     else:
          print(f"Failed to create object pool")
     ...
     for i in range(num_shards):
          shard_index = i
          capi.put("/first/flush_logs", b"",subgroup_type=subgroup_type,subgroup_index=subgroup_index,shard_index=shard_index)
